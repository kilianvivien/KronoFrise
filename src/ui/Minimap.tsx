...
export function Minimap({ width, zoom, pan, setPan, insets }: { insets: ScaleInsets; width: number; zoom: number; pan: number; setPan: (pan: number) => void }): JSX.Element {
  ...
  return <div className="minimap" role="slider" aria-label={M2.minimap} aria-valuemin={Math.round(limits.min)} aria-valuemax={Math.round(limits.max)} aria-valuenow={Math.round(pan)} tabIndex={0}
    ...
    onPointerDown={(e) => { if (drag.current || e.button !== 0) return; const x = e.clientX - e.currentTarget.getBoundingClientRect().left; drag.current = { pointerId: e.pointerId, offset: x >= left && x <= right ? x - (left + right) / 2 : 0 }; e.currentTarget.setPointerCapture(e.pointerId); if (x < left || x > right) go(x); }}
    onPointerMove={(e) => { if (drag.current?.pointerId === e.pointerId) { go(e.clientX - e.currentTarget.getBoundingClientRect().left - drag.current.offset); } }} onPointerUp={(e) => { if (drag.current?.pointerId !== e.pointerId) return; drag.current = null; if (e.currentTarget.hasPointerCapture(e.pointerId)) e.currentTarget.releasePointerCapture(e.pointerId); }} onPointerCancel={(e) => { if (drag.current?.pointerId === e.pointerId) drag.current = null; }} onLostPointerCapture={(e) => { if (drag.current?.pointerId === e.pointerId) drag.current = null; }}>

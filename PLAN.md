...
4. **Dependencies are a closed list:** react, react-dom, zustand, immer, zod, @use-gesture/react, pdf-lib, idb-keyval, uuid (+ dev tooling: vite, vitest, playwright, eslint, typescript). Adding anything else requires asking Kilian first — no drag-and-drop libs, no chart libs, no UI kits, no date libs (core/dates.ts is the date library).
